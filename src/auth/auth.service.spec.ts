...
import { EmailNotVerifiedError, InvalidCredentialsError, InvalidOtpError } from './auth.errors';
...
describe('AuthService', () => {
  ...
  describe('register', () => {
    const dto = { email: 'new@test.com', nickname: 'New', tag: 'newuser', password: 'password123' };
    ...
    it('should create user and send OTP email on success', async () => {
      ...
      mockUserService.create.mockResolvedValue(makeUser({ id: 2, email: dto.email }));
      ...
      expect(mockUserService.update).toHaveBeenCalledWith(2, expect.objectContaining({ otp: expect.stringMatching(/^\d{6}$/) }));
      expect(mockMailService.send).toHaveBeenCalledWith(expect.objectContaining({ to: dto.email }));
    });

    it('should throw UserAlreadyExistsError when email is taken', async () => {
      mockUserService.findOne.mockResolvedValueOnce(makeUser()).mockResolvedValueOnce(null);

      await expect(service.register(dto)).rejects.toThrow(UserAlreadyExistsError);
      expect(mockUserService.create).not.toHaveBeenCalled();
    });

    it('should throw UserAlreadyExistsError when tag is taken', async () => {
      mockUserService.findOne.mockResolvedValueOnce(null).mockResolvedValueOnce(makeUser());
  ...
  describe('login', () => {
    ...
    it('should throw EmailNotVerifiedError when email is not verified', async () => {
      mockUserService.findByEmailOrTag.mockResolvedValue(makeUser({ emailVerified: false }));
      jest.mocked(bcrypt.compare).mockResolvedValue(true as never);
  ...
  describe('verifyEmail', () => {
    ...
    it('should verify email and return accessToken', async () => {
      mockUserService.findOne.mockResolvedValue(makeUser({ emailVerified: false, otp: '123456' }));
      mockUserService.update.mockResolvedValue(undefined);

      const result = await service.verifyEmail(dto);

      expect(result).toEqual({ accessToken: 'jwt-token' });
      expect(mockUserService.update).toHaveBeenCalledWith(1, { emailVerified: true, otp: null });
    });

    it('should throw InvalidCredentialsError when user is not found', async () => {
      mockUserService.findOne.mockResolvedValue(null);

      await expect(service.verifyEmail(dto)).rejects.toThrow(InvalidCredentialsError);
    });
  ...
  describe('requestNewOtp', () => {
    ...
    it('should generate a new OTP and send email', async () => {
      ...
      expect(mockUserService.update).toHaveBeenCalledWith(1, { otp: expect.stringMatching(/^\d{6}$/) });
      expect(mockMailService.send).toHaveBeenCalledWith(expect.objectContaining({ to: 'user@test.com' }));
    });

    it('should throw InvalidCredentialsError when user is not found', async () => {
      mockUserService.findOne.mockResolvedValue(null);

      await expect(service.requestNewOtp(dto)).rejects.toThrow(InvalidCredentialsError);
    });
  });
});
